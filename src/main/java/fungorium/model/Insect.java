...
/**
 * Az Insect osztály a rovarokat reprezentálja, amelyek a benőtt Tecton-okon
 * tudnak közlekedni,
...
public class Insect implements FungoriumEntity {
    ...
    /**
     * Visszaadja a {@link Tecton} objektumot, amelyen az Insect a metódus hívásakor
     * állt.
     *
    ...
    public void move(Tecton target) throws IllegalStateException {
        ...
        while (!queue.isEmpty()) {
            ...
            if (targetJunctions.contains(current)) { // Ha megtalaltuk
                if (currentDistance <= currentSpeed) {
                    ...
                    System.out.println(log);
                    TestFramework.logOutput(log);
                    throw new IllegalStateException("A kiválasztott tekton túl messze van, a rovar jelenleg "
                            + this.speed + " távolságot tud megtenni!");
                }
                return;
            }
        ...
        System.out.println(log);
        TestFramework.logOutput(log);
        throw new IllegalStateException("Nincs a kiválasztott tektonhoz vezető gombafonal!");
    }
     ...
     * A resetEffectValues() függvénnyel visszaállítja a hatások attribútumait
     * alapértékeire,
     * majd egy ciklussal végig megy az összes megevett hatást kifejtő spóra
     * tárolóján (eatenAffectingSpores) és kifejteti magán a hatását.
     * Ha a spóra a hatáskifejtés beállítása után, ha a Spóra true-val jelzi, hogy
     * lejárt a hatása. Ilyenkor a rovar kiveszi a rá hatást kifejtő spórák
     * listájából.
     */
    @Override
